...
combination_indices = list(combinations(np.arange(len(dataset_files)), 3))
n_runs = len(combination_indices)
...
for i, combination_indices in enumerate(combination_indices):
    print(">>> ", i)

    y_ensemble_valid = [data[i]['Y_test_predictions'] for i in combination_indices]
    ...
    y_ensemble_test = [data[i]['Y_test_for_fusion_predictions'] for i in combination_indices]
    ...
    eval_classifiers.set_instances([('ResNet ' + str(i + 1)) for i in combination_indices])
